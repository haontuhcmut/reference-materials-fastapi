...
from app.db.model import Transaction, TransactionDetail, Inventory, Product, Material, Warehouse

from app.transaction.schema import TransactionType,CreateTransactionDetail, TransactionDetailModel, TransactionModel
...
class TransactionService:
    ...
    async def create_transaction_detail(self, transaction_data: CreateTransactionDetail, session: AsyncSession):
        try:
            async with session.begin():
                ...
                for index, detail in enumerate(transaction_data.details, start=1):
                    ...
                    new_detail = TransactionDetail(**detail_dict, transaction_id=new_transaction.id)
                    session.add(new_detail)
                    detail_result.append(new_detail)

                    # Inventory available checking
                    statement = select(Inventory).where(Inventory.warehouse_id == detail.warehouse_id,
                                                        Inventory.product_id == detail.product_id if detail.product_id else Inventory.material_id == detail.material_id)
                    result = await session.exec(statement)
                    ...
                    # Export handling
                        ...
                        if not inventory:
                            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Item {index}: Item not found in inventory.")
                        if inventory.quantity < detail.quantity:
                            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Item {index}: Not enough stock.")
                        inventory.quantity -= detail.quantity

            new_transaction_dict = new_transaction.model_dump()
            return {
                "transaction": TransactionModel(**new_transaction_dict),
                "detail": [
