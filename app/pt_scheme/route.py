...
from app.pt_scheme.schema import (
    CreatePTSchemeModel,
...
async def create_scheme(scheme_data: CreatePTSchemeModel, session: SessionDep):
    new_scheme, category_name = await pt_scheme_service.create_scheme(
        scheme_data, session
    )
...
async def update_scheme(
    scheme_id: str, data_update: CreatePTSchemeModel, session: SessionDep
):
    update_result = await pt_scheme_service.update_scheme(
        scheme_id, data_update, session
    )
    if update_result is None:
        raise PTSChemeNotFound()
    updated_scheme, category_name = update_result
    return PTSchemeWithCategoryModel(
        **updated_scheme.model_dump(), category_name=category_name
    )
